...
function StandaloneWorker(options = {}) {
  ...
  function safeClearBackoffInterval (connectTimer)
  {
    if (connectTimer)
    {
  ...
  if (!readStream) {
    ...
    socket.on('error', function saWorker$$socket$errorHandler(e) {
      ...
      if (!dcpConfig.standaloneWorker.quiet)
        console.warn(`${timestamp()} Evaluator ${this.serial} - socket error:`,
  ...
  function beginSession () {
    connected = true
    safeClearBackoffInterval(connectTimer);
  ...
  shutdown = (e) => {
    ...
    try {
      ...
      console.error(`Warning: could not shutdown evaluator connection ${e.code}`, e);
    };
    connected = false;
  }
